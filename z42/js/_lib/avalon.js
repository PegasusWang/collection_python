...
(function(DOC) {
    ...
    function scanTag(elem, vmodels, node) {
        ...
        var a = elem.getAttribute("ms_skip")
        //#360 在旧式IE中 Object标签在引入Flash等资源时,可能出现没有getAttributeNode,innerHTML的情形
        if (!elem.getAttributeNode) {
            return log("warning " + elem.tagName + " no getAttributeNode method")
        }
        var b = elem.getAttributeNode("ms_important")
        var c = elem.getAttributeNode("ms_view")
        if (typeof a === "string") {
            return
        } else if (node = b || c) {
    ...
    function scanAttr(elem, vmodels) {
            ...
            if (attr.specified) {
                if (match = attr.name.match(rmsAttr)) {
                    ...
                    if (events[type]) {
                        ...
                        log("ms_" + type + "已经被废弃,请使用ms_attr_*代替")
                        if (type === "enabled") { //吃掉ms_enabled绑定,用ms_disabled代替
                            type = "disabled"
                            value = "!(" + value + ")"
                        }
                        param = type
        ...
        if (msData["ms_attr_checked"] && msData["ms_duplex"]) {
            log("warning!一个元素上不能同时定义ms_attr_checked与ms_duplex")
        }
        var scanChild = true
    ...
    var bindingExecutors = avalon.bindingExecutors = {
        "attr": function(val, elem, data) {
            ...
            if (method === "css") {
                ...
                // ms_attr_class="xxx" vm.xxx="aaa bbb ccc"将元素的className设置为aaa bbb ccc
                // ms_attr_class="xxx" vm.xxx=false  清空元素的所有类名
                // ms_attr_name="yyy"  vm.yyy="ooo" 为元素设置name属性
                if (boolMap[attrName]) {
                ...
                if (window.chrome && elem.tagName === "EMBED") {
                    ...
                    var comment = document.createComment("ms_src")
        ...
        "if": function(val, elem, data) {
            if (val) { //插回DOM树
                ...
                if (elem.nodeType === 1) {
                    var node = data.element = DOC.createComment("ms_if")
    ...
    var bindingHandlers = avalon.bindingHandlers = {
        ...
        "attr": function(data, vmodels) {
            ...
            if (data.type === "include") {
                ...
                data.startInclude = DOC.createComment("ms_include")
                data.endInclude = DOC.createComment("ms_include-end")
                if (outer) {
        ...
        "duplex": function(data, vmodels) {
            ...
            if (data.evaluator && data.args) {
                ...
                if (elem.msData) {
                    elem.msData["ms_duplex"] = data.value
                }
                data.param.replace(/\w+/g, function(name) {
                    if (/^(checkbox|radio)$/.test(elem.type) && /^(radio|checked)$/.test(name)) {
                        if (name === "radio")
                            log("ms_duplex-radio已经更名为ms_duplex-checked")
                        name = "checked"
                        data.isChecked = true
                    }
                    if (name === "bool") {
                        name = "boolean"
                        log("ms_duplex-bool已经更名为ms_duplex-boolean")
                    } else if (name === "text") {
                        name = "string"
                        log("ms_duplex-text已经更名为ms_duplex-string")
                    }
                    if (casting[name]) {
                        hasCast = true
                    }
                ...
                for (var i in avalon.vmodels) {
                    var v = avalon.vmodels[i]
                    v.$fire("avalon-ms_duplex-init", data)
                }
        ...
        "repeat": function(data, vmodels) {
            ...
            var endRepeat = data.endRepeat = DOC.createComment("ms_repeat-end")
            ...
            updateVModel = function() {
                if ($elem.data("duplex-observe") !== false) {
                    ...
                    if (!Array.isArray(array)) {
                        log("ms_duplex应用于checkbox上要对应一个数组")
                        array = [array]
                    }
    ...
    duplexBinding.SELECT = function(element, evaluator, data) {
        ...
        data.handler = function() {
            ...
            if (Array.isArray(val)) {
                if (!element.multiple) {
                    log("ms_duplex在<select multiple=true>上要求对应一个数组")
                }
            } else {
                if (element.multiple) {
                    log("ms_duplex在<select multiple=false>不能对应一个数组")
                }
            }
    ...
    //如果ms_repeat紧挨着ms_repeat-end，那么就返回ms_repeat-end
    // 取得用于定位的节点。比如group = 3,  结构为
    // <div><!--ms_repeat--><br id="first"><br/><br/><br id="second"><br/><br/><!--ms_repeat-end--></div>
    // 当pos为0时,返回 br#first
    // 当pos为1时,返回 br#second
    // 当pos为2时,返回 ms_repeat-end
