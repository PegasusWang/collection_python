define(["avalon.getModel", "text!./avalon.dialog.html"], function(avalon, sourceHTML) {
    ...
    var widget = avalon.ui.dialog = function(element, data, vmodels) {
        ...
        var len = vmodels.length,
            ...
            confirmVM;
        // 当存在嵌套dialog时，vmodels的length>1(外层dialog view 和用户定义的view，显然用户的配置存在于用户定义的view，也就是vmodels的最后一个数组元素)，而用户为内嵌dialog设置options无法自动应用，因此需要进行一下转换处理，务必确保用户外部定义配置项不可监控
        if (len > 1) {
        ...
        var _footArr = options.template.split("MS_OPTION_FOOTER"),
            _contentArr = _footArr[0].split("MS_OPTION_CONTENT"),
            ...
            _lastFooter = _footArr[1], // foot html
        ...
        var vmodel = avalon.define(data.dialogId, function(vm) {
             ...
             * @param scope: 当存在层中层时，才可能有2个参数，此时scope是用户定义的view的id 
             **/
            ...
            // 将零散的模板(dialog header、dialog content、 dialog foot、 dialog wrapper)组合成完整的dialog
