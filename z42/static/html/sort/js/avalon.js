...
(function(DOC) {
        ...
        getWidgetData: function(elem, prefix) {
            ...
            for (var i in raw) {
                if (i.indexOf(prefix) === 0) {
                    result[i.replace(prefix,"").replace(/([a-z])([A-Z])/,"$1_$2").toLowerCase()] = raw[i]
                }
                //if (i.indexOf(prefix) === 0) {
    ...
    var bindingHandlers = avalon.bindingHandlers = {
        ...
        "repeat": function(data, vmodels) {
            ...
            if (type !== "repeat") {
                log("warning:建议使用ms_repeat代替ms_each, ms_with, ms_repeat只占用一个标签并且性能更好")
            }
            ...
            if (type === "repeat") {
                var startRepeat = DOC.createComment("ms_repeat-start")
                var endRepeat = DOC.createComment("ms_repeat-end")
            ...
            data.fastRepeat = !!node && node.nodeType === 1 && template.lastChild === node && !node.attributes["ms_view"] && !node.attributes["ms_important"]
        ...
        "if": function(data, vmodels) {
            ...
            elem.removeAttribute(data.name)
            if (!data.placehoder) {
                data.msInDocument = data.placehoder = DOC.createComment("ms_if")
            }
            data.vmodels = vmodels
            parseExprProxy(data.value, vmodels, data)
        },
    ...
    // 取得用于定位的节点。在绑定了ms_each, ms_with属性的元素里，它的整个innerHTML都会视为一个子模板先行移出DOM树，
    // 然后如果它的元素有多少个（ms_each）或键值对有多少双（ms_with），就将它复制多少份(多少为N)，再经过扫描后，重新插入该元素中。
