...
(function($) {
    ...
    $.Poshytip = function(elm, options) {
        ...
        this.$tip = $(['<div style="z-index: 10000;" class="',this.opts.className,'">',
                '<div class="tip-inner tip-bg-image"></div>',
                '<div class="tip-arrow tip-arrow-top tip-arrow-right tip-arrow-bottom tip-arrow-left"></div>',
            '</div>'].join(''));//.appendTo(options.body || document.body);
    ...
    $.Poshytip.prototype = {
        ...
        hide: function() {
            if (this.disabled || !this.$tip.data('active'))
                return;
            this.display(true);
            if (this.opts.destroy) this.destroy();
        },
        ...
        refresh: function(async) {
            ...
            if(this.opts.body != document.body){
                if($.isFunction(this.opts.body)){
                    body = this.opts.body.apply(this.$elm)
                }else{
                    body = this.opts.body
                }
                var width = $tip.width()
                $tip.css({left: 0, top: 0, width:width}).appendTo(body);
            }
...
*/
        ...
        calcPos: function() {
            ...
            if (this.opts.alignTo == 'cursor') {
                ...
                var elmOffset = this.$elm.position(),
                    elm = {
                        l: elmOffset.left,
                        t: elmOffset.top,
            ...
            switch (this.opts.alignX) {
                ...
                case 'inner-left':
                    ...
                    break;
                case 'center':
                    pos.l = xC - Math.floor(this.tipOuterW / 2) + this.opts.offsetX;
            ...
            switch (this.opts.alignY) {
                ...
                case 'inner-top':
                    ...
                    break;
                case 'center':
                    pos.t = yC - Math.floor(this.tipOuterH / 2) + this.opts.offsetY + parseInt(this.$elm.css('marginTop'));
                    if (this.opts.keepInViewport) {
    ...
    $.fn.poshytip = function(options) {
        ...
        if (opts.liveEvents && opts.showOn != 'none') {
            var handler,
                deadOpts = $.extend({}, opts, { liveEvents: false , destroy:true});
            switch (opts.showOn) {
                case 'hover':
                    handler = function() {
                        var $this = $(this);
